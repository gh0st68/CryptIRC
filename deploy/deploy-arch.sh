...
ART
    printf "%b" "${NC}"
    echo -e "       ${DIM}🔒 End-to-end encrypted IRC client${NC}   ${GREY}v0.6 · Arch${NC}"
}
...
command -v pacman &>/dev/null || die "This is the Arch installer, but pacman isn't here. On Debian/Ubuntu, run:  sudo bash deploy/deploy.sh"
...
systemctl enable caddy >/dev/null 2>&1 || true              # persist across reboot (pacman doesn't auto-enable services)
...
# ("permission denied") — caught this for real on a fresh Arch box, not by
# reading the code. Re-chown recursively right before the actual restart.
