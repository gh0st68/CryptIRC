...
# Usage: sudo bash deploy/deploy.sh [domain] [email]
#
...
echo ""
echo -e "${CYAN}╔══════════════════════════════════════════════════════╗${NC}"
echo -e "${CYAN}║${NC}${BOLD}              CryptIRC Installer v0.5                  ${NC}${CYAN}║${NC}"
...
if [[ -z "$DOMAIN" ]]; then
    ...
    check_dns() {
        local resolved=""
        # Try dig first, fall back to host, fall back to getent
...
if [[ "$ENABLE_EMAIL" == "true" ]]; then
    ...
    DEBIAN_FRONTEND=noninteractive apt-get install -y postfix libsasl2-dev >/dev/null 2>&1
fi
